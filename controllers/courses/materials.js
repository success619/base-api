...
import { createUploader } from "../../storage/multer/dynamicStorage.js";
import db from "../../database/db.js";
import useBaseTranspiler from "../../hooks/useBaseTranspiler.js";

const materialsRoute = express.Router();
const { JSDOM } = jsdom;
const { PutObjectCommand } = clientS3;
  ...
  try {
    ...
    const dom = new JSDOM(
      DOMPurify.sanitize(content, {
        ADD_TAGS: ["aside"], //removed button too
        ADD_ATTR: ["data-action", "data-index", "style"], //removed content-editable too
      }),
    );
    ...
    images.forEach((img) => {
      ...
      if (file) {
        img.src = isProd
          ? file.location
          : `${process.env.APP_URL}/public/files/materials/images/${file.filename}`;
      } else {
